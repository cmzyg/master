...
$database->select('SELECT fixed_10_status, fixed_price_10, fixed_location_10, fixed_lat_10, fixed_lon_10, fixed_price_10_img, free_text_10 FROM fixed_prices');

$fixed = $database->fetch();

// if journey is inactive - redirect to homepage
if($fixed['fixed_10_status'] == 'Inactive')
{
  $controller->redirect($base_url);
}

$lat = $fixed['fixed_lat_10'];
$longitude = $fixed['fixed_lon_10'];
...
<title>Fixed Journey to <?php echo $fixed['fixed_location_10']; ?></title>
...
google.maps.event.addDomListener(window, 'load', initialize);
  ...
  <input type="text" id="js_hidden_total" name="js_hidden_total" value="Total: &pound;<?php echo number_format($fixed['fixed_price_10'], 2); ?>" readonly />
...
<?php include('includes/header.php'); ?>
  ...
  <div id="content-top" class='fixed-journey-details' style="background-color: white; color: black; width: 33%; height: 300px; float: left;">
    ...
    <ul class="slides">
      <li>
        <div class="accomodation-carousel-item" style="margin-bottom:10px; position:relative;"> <a href="fixed-journey-10"> <img alt="" src="<?php echo $base_url; ?><?php echo $fixed['fixed_price_10_img']; ?>" width="200" height="100" /> </a>
        <div class="title" style='font-size: 14pt'> <a href="fixed-journey-10"><?php echo $fixed['fixed_location_10']; ?><div class='freetext'><?php echo $fixed['free_text_10']; ?></div></a> </div>
        <div class="price">&pound;<?php echo $fixed['fixed_price_10']; ?>
...
<!-- end vehicles -->
  ...
  <div id="content" class="six columns">
      ...
      <input type="hidden" name="free_text" value="<?php echo $fixed['fixed_location_10'] . ' ' . $fixed['free_text_10']; ?>" />
      <input type="hidden" name="destination" value="<?php echo $fixed['fixed_location_10']; ?>" style="width:100%" />
      <input type="hidden" name="payment" value="<?php echo $fixed['fixed_price_10']; ?>" />
