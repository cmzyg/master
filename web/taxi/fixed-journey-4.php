...
$database->select('SELECT fixed_4_status, fixed_price_4, fixed_location_4, fixed_lat_4, fixed_lon_4, fixed_price_4_img, free_text_4 FROM fixed_prices');

$fixed = $database->fetch();

// if journey is inactive - redirect to homepage
if($fixed['fixed_4_status'] == 'Inactive')
{
  $controller->redirect($base_url);
}

$lat = $fixed['fixed_lat_4'];
$longitude = $fixed['fixed_lon_4'];
...
<title>Fixed Journey to <?php echo $fixed['fixed_location_4']; ?></title>
...
google.maps.event.addDomListener(window, 'load', initialize);
  ...
  <input type="text" id="js_hidden_total" name="js_hidden_total" value="Total: &pound;<?php echo number_format($fixed['fixed_price_4'], 2); ?>" readonly />
...
<?php include('includes/header.php'); ?>
  ...
  <div id="content-top" class='fixed-journey-details' style="background-color: white; color: black; width: 33%; height: 300px; float: left;">
    ...
    <ul class="slides">
      <li>
        <div class="accomodation-carousel-item" style="margin-bottom:10px; position:relative;"> <a href="fixed-journey-4"> <img alt="" src="<?php echo $base_url; ?><?php echo $fixed['fixed_price_4_img']; ?>" width="200" height="100" /> </a>
        <div class="title" style='font-size: 14pt'> <a href="fixed-journey-4"><?php echo $fixed['fixed_location_4']; ?><div class='freetext'><?php echo $fixed['free_text_4']; ?></div></a> </div>
        <div class="price">&pound;<?php echo $fixed['fixed_price_4']; ?>
...
<!-- end vehicles -->
  ...
  <div id="content" class="six columns">
      ...
      <input type="hidden" name="free_text" value="<?php echo $fixed['fixed_location_4'] . ' ' . $fixed['free_text_4']; ?>" />
      <input type="hidden" name="destination" value="<?php echo $fixed['fixed_location_4']; ?>" style="width:100%" />
      <input type="hidden" name="payment" value="<?php echo $fixed['fixed_price_4']; ?>" />
