...
$count_bookings = $bookings->countBookingsByDate($date);

$count = 0;
$bookings_array = array();

$bookings_list = $bookings->getBookingsByDate($date);

foreach($bookings_list as $rows)
{
	$bookings_array[$count]['booking_date']  = date('d-m-Y H:i:s', strtotime($rows['booking_date']));
	$bookings_array[$count]['return_date']   = $rows['return_date'];
	...
	$bookings_array[$count]['actual_pickup'] = $rows['actual_pickup'];
	...
	$bookings_array[$count]['actual_destination'] = $rows['actual_destination'];
	$bookings_array[$count]['meet_and_greet']     = $rows['meet_and_greet_service'];
    $count++;
}
...
<title><?php echo date('d-m-Y', strtotime($date)); ?> Bookings</title>
<link rel='stylesheet' media='all' href='../../../assets/css/datepicker/styles.css' />
<link rel='stylesheet' media='all' href='../../../assets/css/datepicker/jquery.ui.all.css' />
...
<script src="../../../assets/js/datepicker/jquery-1.10.2.js"></script>
<script src="../../../assets/js/datepicker/jquery.ui.core.js"></script>
<script src="../../../assets/js/datepicker/jquery.ui.datepicker.js"></script>
...
<div class="wrap">
<h2>
Bookings on date <?php echo date('d-m-Y', strtotime($date)); ?> 
</h2>
<div class='description'>If customer cancels a booking you have already accepted, please change booking status to cancelled. An email will automatically be sent to the customer to confirm cancellation. Accepted bookings will automatically be marked ‘Completed’ 1 day after the journey date.</div>
<br />
<ul>
...
<table class="wp-list-table widefat fixed users" cellspacing="0">
	...
	<tr>
		...
		<th scope='col'><strong>Travel Dates</strong></th>
	...
	<tr id='user-1' class="alternate">
		...
		<td class='<?php echo $row['status']; ?>'>Travelling on: <br /><?php echo $row['booking_date']; ?><br />
			<?php if($row['return_date'] !== '0000-00-00 00:00:00') { ?>Returning on: <br /><?php echo date('d-m-Y H:i:s', strtotime($row['return_date'])); ?><br /><?php } ?>
		...
		<td class='<?php echo $row['status']; ?>'><strong>From:</strong> <?php echo $row['from']; ?><br /> <strong>To:</strong> <?php echo $row['to']; ?><br /><strong>Pickup:</strong> <?php echo $row['actual_pickup']; ?><br /><strong>Destination:</strong> <?php echo $row['actual_destination']; ?> <?php if($row['meet_and_greet'] == 1) { echo '<br /><strong>+ Meet and Greet</strong>'; } ?></td>
		<td class='<?php echo $row['status']; ?>'><button type="button" class='more_info' style='cursor:pointer'>Open</button></td>
	</tr>

	<tr id='user-1' class="alternate" class="more_info_2" style="display:none">
