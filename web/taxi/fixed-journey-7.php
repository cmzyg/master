...
$database->select('SELECT fixed_7_status, fixed_price_7, fixed_location_7, fixed_lat_7, fixed_lon_7, fixed_price_7_img, free_text_7 FROM fixed_prices');

$fixed = $database->fetch();

// if journey is inactive - redirect to homepage
if($fixed['fixed_7_status'] == 'Inactive')
{
  $controller->redirect($base_url);
}

$lat = $fixed['fixed_lat_7'];
$longitude = $fixed['fixed_lon_7'];
...
<title>Fixed Journey to <?php echo $fixed['fixed_location_7']; ?></title>
...
google.maps.event.addDomListener(window, 'load', initialize);
  ...
  <input type="text" id="js_hidden_total" name="js_hidden_total" value="Total: &pound;<?php echo number_format($fixed['fixed_price_7'], 2); ?>" readonly />
...
<?php include('includes/header.php'); ?>
  ...
  <div id="content-top" class='fixed-journey-details' style="background-color: white; color: black; width: 33%; height: 300px; float: left;">
    ...
    <ul class="slides">
      <li>
        <div class="accomodation-carousel-item" style="margin-bottom:10px; position:relative;"> <a href="fixed-journey-7"> <img alt="" src="<?php echo $base_url; ?><?php echo $fixed['fixed_price_7_img']; ?>" width="200" height="100" /> </a>
        <div class="title" style='font-size: 14pt'> <a href="fixed-journey-7"><?php echo $fixed['fixed_location_7']; ?><div class='freetext'><?php echo $fixed['free_text_7']; ?></div></a> </div>
        <div class="price">&pound;<?php echo $fixed['fixed_price_7']; ?>
...
<!-- end vehicles -->
  ...
  <div id="content" class="six columns">
      ...
      <input type="hidden" name="free_text" value="<?php echo $fixed['fixed_location_7'] . ' ' . $fixed['free_text_7']; ?>" />
      <input type="hidden" name="destination" value="<?php echo $fixed['fixed_location_7']; ?>" style="width:100%" />
      <input type="hidden" name="payment" value="<?php echo $fixed['fixed_price_7']; ?>" />
